 ...
 * edit by pengyi since  2015-11-26
...
!function (window, $, undefined) {
    ...
    var SVGMap = function () {
        ...
        return SVGMap.prototype.options = {
            ...
            mapWidth: 500,
            mapHeight: 400,
            stateColorList: ["#2770B5", "#429DD4", "#5AABDA", "#1C8DFF", "#70B3DD", "#C6E1F4", "#EDF2F6", "#008B00"],
            ...
            var e;
            "VML" == Raphael.type ? (d.innerHTML = "<rvml:group style='position : absolute; width: 1000px; height: 1000px; top: 0px; left: 0px' coordsize='1000,1000' class='rvml' id='vmlgroup_" + a + "'></rvml:group>",
            e = document.getElementById("vmlgroup_" + a)) : (d.innerHTML = '<div class="svggroup"></div>',
            e = d.getElementsByClassName("svggroup")[0]);
            var g,
                f = new Raphael(e, b, c);
            return "SVG" == Raphael.type ? f.canvas.setAttribute("viewBox", "0 0 " + b + " " + c) : g = d.getElementsByTagName("div")[0],
                f.changeSize = function (a, h, i, j) {
                    ...
                    if ("VML" == Raphael.type) {
                        ...
                        var t;
                        t = n > o ? 1e3 * o / b : 1e3 * n / c,
                        t = parseInt(t),
                        e.style.width = t + "px",
                        e.style.height = t + "px", j &&
                        (e.style.left = parseInt((a - o) / 2) + "px",
                        e.style.top = parseInt((h - n) / 2) + "px"),
                        g.style.overflow = "visible"
                    }
                    j &&
                    (o = a, n = h),
                    d.style.width = o + "px",
                    d.style.height = n + "px", f.setSize(o, n), i &&
                    (d.style.position = "absolute",
                    d.style.left = parseInt((a - o) / 2) + "px",
                    d.style.top = parseInt((h - n) / 2) + "px")
            ...
            function nameSwitch(id){
                switch(id){
                    case '8':return 'dongbei';break;
                    case '4':return 'huabei';break;
                    case '5':return 'huadong';break;
                    case '7':return 'xibei';break;
                    case '3':return 'huazhong';break;
                    case '2':return 'huanan';break;
                    case '6': return 'xinan'; break;
                    default: break;
                }
                return id;
            }
            ...
            function sharpHover(a, b) {
                if (opt.hoverRegion == b.id) {
                    return opt.timeTimer = 1, clearTimeout(b.timer), void 0;
                }
                var areaname=nameSwitch(b.id);
                // add at 2015-11-27
                $("." + areaname + "_grey").css("display", "none");
                $("." + areaname + "_white").css("display", "block");

                if (opt.external &&
                    "undefined" != typeof self.externalData[b.id].eventHoverLock &&
                    0 != self.externalData[b.id].eventHoverLock ||
                    -1 == opt.clickedRegion.indexOf(b.id) &&
                    b.animate({ fill: stateColor[b.id].hoverColor }, 150), opt.showTip) {

                    opt.timeTimer = 1, 0 == $("#MapTip").length &&
            ...
            function sharpOut(a, b) {
                opt.timeTimer = 0, opt.hoverRegion = "", b.timer = setTimeout(function () {
                    opt.hoverRegion != b.id &&
                    (opt.external && "undefined" != typeof self.externalData[b.id].eventHoverLock && 0 != self.externalData[b.id].eventHoverLock || -1 == opt.clickedRegion.indexOf(b.id) && b.animate({
                        fill: stateColor[b.id].initColor
                    }, 100), opt.showTip &&
                    1 != opt.timeTimer && $("#MapTip").remove())
                }, 100);
                var flag = false;
                for (var re in opt.clickedRegion) {
                    if (opt.clickedRegion[re] == b.id) {
                        flag= true;
                    }
                }
                if (!flag) {
                    var areaname = nameSwitch(b.id);
                    $("." + areaname + "_grey").css("display", "block");
                    $("." + areaname + "_white").css("display", "none");
                }
            }
            // 鼠标点击事件
            function sharpClick(a, b) {
                ...
                0 != opt.clickColorChange && (opt.external &&
                "undefined" != typeof self.externalData[b.id].eventClickLock &&
                0 != self.externalData[b.id].eventClickLock ||
                (-1 == opt.clickedRegion.indexOf(b.id) ? (opt.clickedRegion.push(b.id),
                //填充颜色
                b.animate({
                    fill: stateColor[b.id].selectedColor
                }, 150),
                opt.clickCallback(stateData, b)) : (opt.clickedRegion.remove(b.id), opt.external &&
                "undefined" != typeof self.externalData[b.id].eventHoverLock &&
                0 != self.externalData[b.id].eventHoverLock ? b.animate({
                    fill: stateColor[b.id].initColor
                }, 150) : b.animate({
            ...
            function deselctAll() {
                ...
                for (var i = 0; i < len; i++) {
                    var b = backup[i];
                    if (b.attr("fill") != "#2D2D30") {
                        opt.clickedRegion.remove(b.id);
                        b.attr({ fill: "#2D2D30" });
                    }
                }
                // add at 2015-11-27
                $('.area-marker > img:first-child').show();
                $('.area-marker > img:nth-child(2)').hide();
            }
                    ...
                    success: function (a) {
                        ...
                        c.find("stateData").each(function (a) {
                            var b = $(this),
                                c = b.attr("stateName");
                            stateData[c] = {};
                            for (var a = 0, d = opt.stateDataAttr.length; d > a; a++) {
                                stateData[c][opt.stateDataAttr[a]] = b.attr(opt.stateDataAttr[a])
                            }
                        });
                    }
                });
            ...
            for (var state in mapConfig.shapes) {
                ...
                stateData[state] && stateData[state].diabled ? obj.attr({
                    ...
                    opt.external && "undefined" != typeof self.externalData[obj.id].eventHoverLock &&
                    1 == self.externalData[obj.id].eventHoverLock || sharpOut(a, this)
                }).click(function (a) {
                    opt.external && "undefined" != typeof self.externalData[obj.id].eventClickLock &&
                    1 == self.externalData[obj.id].eventClickLock || sharpClick(a, this)
                }));
                backup.push(obj);
            }
