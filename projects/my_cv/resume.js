$(function () {
    $(".info-nav").on('click', function (evt) {
        if (evt.target.nodeName.toUpperCase() == "LI") {
            ...
            //$("#"+linkTarget).show();
            var scrollNum = target.attr('data-top') || $('#' + target.attr('link-target')).offset().top;
    ...
    $(window).load(function () {
        var scrollTop = [];
        $.each($('.info-nav li'), function (index, item) {
            if (!$(item).attr('data-top')) {
                var top = $('#' + $(item).attr('link-target')).offset().top;
                scrollTop.push(top);
        ...
        $(window).scroll(function () {
            waitForFinalEvent(function () {
                ...
                if (nowTop> scrollTop[length - 1]) {
                    index = length
                } else {
                    for (var i = 0; i < length; i++) {
                        if (nowTop<= scrollTop[i]) {
                            index = i
                            break;
                        }
                    }
                ...
                $('.info-nav li').eq(index).addClass('active')
            })
        });
    });
    ...
    function canvasAnimation() {
        ...
        function text(n) {
            ...
            context.strokeStyle = "#49f"; //设置描边样
            context.font = '27px "Roboto Light"'; //设置字体大小和字体
            //绘制字体，并且指定位置
